...
CELLS = [
    ...
    md("## 步驟 1|環境設定(中文字型)",
       "Colab 沒有微軟正黑,圖上的中文會變豆腐 → 裝 Noto CJK。(TensorFlow 已內建)"),
    code("# 中文字型(避免圖變豆腐);TensorFlow 已內建",
    ...
    md("## 步驟 5|補訓練曲線/混淆矩陣 + 打包所有圖",
       "Colab 在這裡的價值就是這些**圖**——拿去貼進你自己的投影片(REPORT.md 標了每張該放哪頁)。",
       "(這支筆記本不負責組投影片;想要自動草稿 pptx 見最後備註。)"),
    code("!python report/make_report_figs.py    # 訓練曲線 + 混淆矩陣 + demo 示意",
         "import shutil",
         "shutil.make_archive('/content/figures', 'zip', 'experiments/figures')",
         "print('已打包:/content/figures.zip')"),

    md("## 步驟 6|看圖 + 下載(貼進你自己的投影片)"),
    code("from IPython.display import Image, display",
         "import glob",
         "for f in sorted(glob.glob('experiments/figures/*.png')):",
         "    print(f); display(Image(f, width=640))"),
    code("from google.colab import files",
         "files.download('/content/figures.zip')   # 一包帶走所有圖"),

    md("## (可選)想要 15 頁自動草稿 pptx?",
       "投影片建議自己掌控(貼步驟 6 的圖到你的 deck)。但若想要一份機器生的草稿當起點,跑這兩格:"),
    code("!pip -q install python-pptx",
